...
SELECT
  ...
  (
    SUM(purchased_credits) - (
      ...
      WHERE 
      	"COURSE_BOOKING".user_id = user_id 
...
WHERE
  "SKILL".name = '重訓'
ORDER BY
  "COACH".experience_years DESC; -- 降冪排序
...
WHERE
  "CREDIT_PURCHASE".created_at >= '2024-11-01 00:00:00'
...
SELECT
  COUNT(DISTINCT("COURSE_BOOKING".user_id)) AS 預約會員人數 -- DISTINCT 為不重複資料
FROM
  "COURSE_BOOKING"
WHERE
